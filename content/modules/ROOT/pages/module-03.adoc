...
The Terminal is logged into your bastion host - you will need to log into node1-{guid}.example.com.

[source,sh,role=execute]
----
ssh lab-user@node1-{guid}.example.com
----
...
`test_scan: true` to:`test_scan: false`.  
...
Locate your system. + 
Reminder: your system name is: node1-{guid}.example.com
...
image::Malware_MatchStatus_Options.png[Vuln_SummaryBar,100%,100%]
...
Since this is a Malware detection test, scroll back up a little and set the status of each match as `Malware detection test`.  That clears the `New matches` flag which looks like a bell. +
If desired you could type a note into each match to give more details on the test.
