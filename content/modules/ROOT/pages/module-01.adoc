...
Customers have been specifically asking for Insights on premise for several years, but to really draw a distinction between what Red Hat manages on the Hybrid Cloud Console and what you would manage in your own environment, throughout the lab guide you will see this new feature referred to as `self-managed Insights`.

This is a podman container that we will need to install on the Satellite which in turn will handle the Insights analysis.   Once this is configured your Satellite will no longer be sending any data to console.redhat.com - the local Satellite handles the analysis without sending any data.  For this technical preview, only the Insights advisor service is included, to provide configuration analysis for your RHEL hosts.

* Use the same Satellite user interface to identify and resolve issues found on hosts using the Insights advisor service, without sending any data thanks to self-managed Insights capabilities with Satellite. 
* Recap what you achieved and learn a little about what is coming next.


### Insights connectivity options.
With the introduction of self-managed Insights capabilities in Satellite, there are now three high level modes for Insights.
...
* Self-managed Insights on Satellite (Technical preview feature).  Insights data is processed on a Satellite server.  No data is sent to Red Hat including data for subscription management.  
...
=== Self-managed Insights on Satellite - a new technical preview feature!
This lab introduces a tech preview capability, self-managed Insights on Satellite.  
Insights will use Red Hat Satellite to process and deliver the self-managed Insights capability.
In its tech preview state, only the Adviser capability of Insights will be available through the Satellite user interface. 
Other services and capabilities may be added in future releases, but feature parity with Insights at the https://console.redhat.com/[Red Hat Hybrid Cloud Console^] should not be expected.
Self-managed Insights does not send any data outside of the Satellite for the processing of Insights data.

In the image below you see a representation of the lab environment with Self-managed Insights enabled.  The Insights advisor capability is a podman container on the Satellite server. When the two RHEL hosts connected to the Satellite perform an analysis, that analysis is done on the Satellite server, in the container, without sending any data to Red Hat.
