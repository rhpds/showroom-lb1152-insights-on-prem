= Predictive and proactive recommendations in Satellite while connected.
...
Please use a web browser to navigate to your Satellite’s user interface. +
...
The page will be empty - We want to sync the Recommendations with the ones on the Hybrid Cloud Console.
...
After a few seconds the view will update and Advisor recommendations will be displayed in the Satellite UI.  These are the same ones that you would see in the Advisor service of the Hybrid Cloud Console.  
...
NOTE: While you see results from both hosts, only work on the RHEL 9.1 host in this module.  You  will use the 9.2 host in the next module.

== Recommendations on your system

Let’s look at a host specific view. +

* In the left hand navigation menu of Red Hat Satellite, slick `Hosts → All Hosts` +
* Look at the recommendations on the RHEL 9.1 host. +
* You should see 4 recommendations listed in the table. +
* Click the host name.
* The overview shows you details about the host.  One of the widgets shown is called `Total risks`. (You will likely need to scroll down the page to see this widget.) +
...
* Scroll back to the top of the page if necessary, and click the `Insights` tab. +
...
To fix this, we install then apply the tuned profile. + 
The step by step directions are included, or you can generate an ansible playbook inside of Insights. +
Return to Red Hat Satellite and we will perform a remediation of this issue.

=== Fixing an issue that Insights finds

To start the process of fixing an issue, select the checkbox next to the recommendation. 
...
This job may take 3-5 minutes to complete.
...
On the Satellite left hand navigation bar, select `Insights → Recommendations`.+
It is likely in this lab that the recommendations have not been synchronized from Insights on the Hybrid Cloud Console, so you can manually do this by clicking the three dots and selecting `Sync recommendations`.
