...
While this is a Red Hat Lightspeed advisor in Satellite lab, this software is only a part of the overall Red Hat Lightspeed offering.  
In this module and the next you will explore some of the Red Hat Lightspeed capabilities on the https://console.redhat.com[Hybrid Cloud Console^].

You will start by evaluating common vulnerabilities and exposures (CVEs) that are present in your environment and on your system using the Red Hat Lightspeed vulnerability service.

There are thousands of CVEs - Red Hat Lightspeed helps you to focus on CVE information that pertains to Red Hat systems and some CVEs have gone through additional evaluation to share impact specific to the version of RHEL your system is running.
...
[start=3]
3. On the main page locate the `Red Hat Enterprise Linux` tile in the upper left and click https://console.redhat.com/insights/[RHEL^]. +
...
You should know that the exported data is WYSIWYG (what you see is what you get). +
If you apply any filters to the view, those filters will affect what is exported.
...
You can add this fix to an existing playbook or you can create a new playbook.  Red Hat Lightspeed defaults to Create new playbook.   
...
The `Execute` button is unavailable.  Your user in this lab does not have permission to execute remediation.  No user gets this permission by default - it has to be explicitly provided, With so many people taking this lab at the same time we have opted to not enable this feature. +

We do have an interactive demo that can show you how this works, end to end that is at the end of this module. +
[start=8]
...
2. Complete the report wizard +
* If desired you can change the `Report title`.
* Select `RHEL 9.5` (The Satellite servers are running on thie version), `RHEL 9.1` (Your RHEL nodes are running on this version), and `RHEL 9.0` (you don't have any systems running this version of RHEL).
...
* You can export to PDF of CSV.  Click `Export PDF`.
